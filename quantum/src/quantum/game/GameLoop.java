...
public strictfp class GameLoop {
	...
	public GameLoop (Client client, Simulation sim, boolean shader_off) {
		...
		try {
			...
			Gdx.app.log("[GameLoop]", "couldn't create recorder: ", e1);
			recorder = null;
		}
	 ...
	 * @param engine
	 * @param canvas */
	public void update (boolean blob) {
	...
	protected void update () {
		try {
			...
			Gdx.app.log("[GameLoop]", "error in update: " + e.getMessage() + "\n", e);
	...
	public void render () {
		...
		if (gui == null) gui = new GameInterface(this, sim, renderer);

		renderer.render(sim, gui);
