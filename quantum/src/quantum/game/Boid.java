...
//		GL11.glVertex2f(-1 * Constants.BOID_SIZE, 0.5f * Constants.BOID_SIZE);
//		GL11.glVertex2f(-1 * Constants.BOID_SIZE, -0.5f * Constants.BOID_SIZE);
//		GL11.glVertex2f(1 * Constants.BOID_SIZE, 0);
	}
	...
	public void update () {
		...
		if (force.len() > Constants.BOID_MAX_FORCE) force.nor().scl(Constants.BOID_MAX_FORCE);

		tmp.set(vel);

		vel.add(force);
		if (vel.len() > getMaxSpeed()) vel.nor().scl(getMaxSpeed());
	...
	private void updateAttack () {
		...
		tmp.sub(pos).nor().scl(getMaxSpeed());
	...
	protected void updateSeek () {
		...
		tmp.sub(pos).scl(getMaxSpeed());
		force.set(tmp).sub(vel);
	}

	protected void updateFlee () {
		tmp.set(pos).sub(sim.getObject(target).pos).scl(getMaxSpeed());
		force.set(tmp).sub(vel);
	}

	protected void updateArrive () {
		if (pos.dst2(sim.getObject(target).pos) < 100) behaviour = SteeringBehaviour.None;
		tmp.set(sim.getObject(target).pos).sub(pos).scl(getMaxSpeed());
		force.set(tmp).sub(vel);
	}
...
// }
	...
	protected void updateCircle () {
		...
		if (len2 < planet.getRadius() + Constants.BOID_MIN_ORBIT) {
			if (len2 < planet.getRadius() + Constants.BOID_MIN_ORBIT) vel.scl(0.93f);
	...
	protected void updateNone () {
		...
		vel.scl(Constants.BOID_DECCELARATION);
		force.set(0, 0);
	}
