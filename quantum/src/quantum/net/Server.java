...
public strictfp class Server {
		...
		public void run () {
			...
			while (true) {
				...
				try {
					...
					shutdown("listening socket error: " + e.getMessage());
				}
				...
				try {
					...
					//
					FileHandle internal = Gdx.files.internal("dat/maps");
					File dir = internal.file();
					...
					for (String file : files) {
						...
						try {
							...
							Gdx.app.log("[Server]", "error while loading file '" + file + "'");
						}
					}
					...
					Gdx.app.log("[Server]", "client connection attempt failed: ", e);
				}
			}
	...
	strictfp class Lobby implements Runnable {
		...
		public void addClient (Client client) {
			synchronized (clients) {
				clients.add(client);
				broadcastPlayerList();
				Gdx.app.log("[Server]", "added client: " + client.getPlayer().getId() + ", " + client.getPlayer().getId());
			}
		}
		...
		public void run () {
			...
			try {
				...
				shutdown("couldn't create datagram socket: " + e1.getMessage());
			}
			...
			Gdx.app.log("[Server]", "finished loading files, sending list to client");
			MapListMessage map_msg = new MapListMessage(filesList.toArray(new String[0]), nameList.toArray(new String[0]));
				...
				synchronized (clients) {
						...
						try {
							...
							if (msg != null) {
								...
								if (msg instanceof ReadyMessage) {
									...
									Gdx.app.log("[Server]", "client " + client.getPlayer().getId() + "(" + client.getPlayer().getId()
										+ ") is ready");
								}
								...
								if (msg instanceof MapImageMessage) {
									try {
										...
										Simulation sim = new Simulation(false);
										DataInputStream in = new DataInputStream(new FileInputStream("dat/maps/"
											+ image_msg.getName()));
										sim.readState(in);
										in.close();
										...
										Gdx.app.log("[Server]", "couldn't send MapImageMessage: ", ex);
									}
								}
							...
							Gdx.app.log("[Server]", "disconnected client due to error: ", e);
							disconnected.add(client);
						}
					...
					if (udp_timer.getElapsedSeconds() > 4) {
						try {
							...
							Gdx.app.log("[Server]", "couldn't broadcast server identity: ", e);
	...
	strictfp class Game implements Runnable {
		...
		public Game () {
			Gdx.app.log("[Server]", "starting game");
			thread = new Thread(this);
				...
				while (true) {
					...
					for (Client client : clients) {
						try {
							...
							if (msg != null) {
								...
								if (msg instanceof CommandBufferMessage) {
									...
									if (cmd_msg.getCommandBuffer().getTurnId() != buffer.getTurnId()) {
										Gdx.app.log("[Server]","received buffer for unexpected turn "
								...
								if (msg instanceof GameOverMessage) {
									...
									try {
										...
										Gdx.app.log("[Server]","couldn't restart server: " , e);
										System.exit(0);
									}
							...
							Gdx.app.log("[Server]", "removing client " + client.getPlayer().getName() + ": " , e);
							disconnected.add(client);
					...
					if (clients.size() <= 0) {
						...
						Gdx.app.log("[Server]", "restarting server");
						try {
							login = new Login(port);
						} catch (Exception e) {
							Gdx.app.log("[Server]","couldn't restart server: ", e);
		...
		private Simulation loadLevel () {
			...
			if (votes.size() == 0) {
				...
				MyRandom rand = new MyRandom(System.nanoTime());
				String map_file = files[(int)rand.rand(0, files.length - 1)];

				try {
					DataInputStream dataInputStream = new DataInputStream(new FileInputStream("dat/maps/"
						+ map_file));
					sim.readState(dataInputStream);
					dataInputStream.close();
				} catch (Exception e) {
					Gdx.app.log("[Server]","couldn't load '" + map_file + "': " , e);
					e.printStackTrace();
				}
			} else {
				...
				try {
					sim.readState(new DataInputStream(new FileInputStream("dat/maps/" + file)));
				} catch (Exception e) {
					Gdx.app.log("[Server]","couldn't load '" + file + "': " , e);
					e.printStackTrace();
				}
			}
...
// return sim2;
			...
			Planet p1 = new Planet(sim, new Vector2(0, 0), 200, 1f, 0.1f, 1f, 20);
			Planet p2 = new Planet(sim, new Vector2(5000, 0), 200, 0.5f, 0.5f, 0.5f, 50);
			Planet p3 = new Planet(sim, new Vector2(3000, 3000), 200, 0.5f, 0.5f, 0.5f, 30);
...
// sim.addObject( p5 );
	...
	public Server (int port, String name, boolean testing, String external_ip) throws Exception {
		...
		lobby = new Lobby();
		this.name = name;
		Gdx.app.log("[Server]", "quantum server started, port " + port);
	}
...
// }
	...
	public void sendPlayerList (Client c) {
		synchronized (clients) {
			...
			try {
				...
				Gdx.app.log("[Server]", "couldn't send PlayerListMessage to client " + c.getPlayer().getName() + ": " + e.getMessage());
				e.printStackTrace();
			}
		}
	}
	...
	public void removeClients (List<Client> disconnected_clients) {
		synchronized (clients) {
			...
			for (Client client : disconnected_clients) {
				Gdx.app.log("[Server]", "disconnected client: " + client.getPlayer().getName() + ", " + client.getPlayer().getId());
				client.dispose();
				for (Player player : client.getPlayers()) {
					...
					for (Player player : disconnected_client.getPlayers()) {
						...
						for (Client client : clients) {
							...
							try {
								...
								Gdx.app.log("[Server]", "disconnected player: " + client.getPlayer().getName() + ", "
	...
	public void shutdown (String message) {
		Gdx.app.log("[Server]", "SHUTTING DOWN: " + message);
		login.stop();
		lobby.stop();
